...
namespace ERP.Web.Areas.Erp.Common
{
    public class ErpHelper
    {
        ...
        public static dynamic GetEditUrls(string controller,object extend=null)
        {
            ...
            expando["getdetail"] =  string.Format("/api/erp/{0}/getdetail/",controller);
            expando["getmaster"] =  string.Format("/api/erp/{0}/geteditmaster/",controller);
            expando["edit"] =  string.Format("/api/erp/{0}/edit/",controller);
            expando["audit"] =  string.Format("/api/erp/{0}/audit/",controller);
            expando["getrowid"] =  string.Format("/api/erp/{0}/getnewrowid/",controller);
        ...
        {
            var expando = (IDictionary<string, object>)new ExpandoObject();
            expando["query"] = string.Format("/api/erp/{0}",controller);
            expando["remove"] = string.Format("/api/erp/{0}/", controller);
            expando["billno"] = string.Format("/api/erp/{0}/getnewbillno", controller);
            expando["audit"] = string.Format("/api/erp/{0}/audit/", controller);
            expando["edit"] = string.Format("/erp/{0}/edit/", controller);
            if (extend!=null)
                EachHelper.EachObjectProperty(extend, (i, name, value) => { expando[name] = value; });
