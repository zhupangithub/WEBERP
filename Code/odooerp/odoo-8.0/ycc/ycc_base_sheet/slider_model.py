...
class ycc_slider_model(osv.Model):
    _name = "ycc.slidermodel"
    # _log_access = False
    _description = "ycc.slidermodel"
    _inherit = ['mail.thread']
    _columns = {
        'name': fields.many2one('product.template', u'虚拟拉头', required=True, domain="[('pro_slider_active','=',True)]"),
        'slider_bentou': fields.many2one('product.template', u'本头', required=True,
                                         domain="[('pro_typeid','=','BT(本头)'),('pro_item_active','=',False)]"),
        'slider_key': fields.many2one('product.template', u'连接件',
                                      domain="[('pro_typeid','=','LJJ(连接件)'),('pro_item_active','=',True)]"),
        'slider_pian': fields.many2one('product.template', u'拉片',
                                       domain="[('pro_typeid','=','LP(拉片)'),('pro_item_active','=',False)]",
                                       required=True),
        'tz_name': fields.many2one('product.template', u'涂装拉头', domain="[('pro_slider_active','=',True)]"),
        'tz_state': fields.char(''),
    ...
    def dt_done(self, cr, uid, ids, context=None):
        try:
            ...
            slider_line = self.pool.get('ycc.slidermodel').browse(cr, uid, ids)  # 订单行
            cr.execute("select id from product_product where product_tmpl_id=%s and id!=%s", (slider_line.name.id, 0))
            product = cr.fetchone()
            state = 'done'
            message = '单据已完成'
            self.dt_action(cr, uid, ids, state, message, context=context)
            if slider_line.name.name != False and slider_line.tz_name.name == False:
                names = slider_line.name.name + slider_line.slider_pian.name
                            ...
                            for bom in cr.fetchall():
                                ...
                                for bom_line in cr.fetchall():  # 遍历BOM明细中的数据
                                    ...
                                    if bom_line[19] == u'LP(拉片)':
                                        cr.execute(sql_str,
                                                   (slider_line.slider_pian.id, 0))
                                        indexs = cr.fetchone()
                                    cr.execute(add_mrp_bom_line_sel,
                                               (indexs, bom_line[10], bom_id_max_t, bom_line[18]))
            if slider_line.name.name != False and slider_line.tz_name.name != False:
                cr.execute("select id from product_product where product_tmpl_id=%s and id!=%s",
                       (slider_line.tz_name.id, 0))
                product2 = cr.fetchone()
                cr.execute("select id from mrp_bom where product_tmpl_id=%s and id!=%s",(slider_line.name.id,0))
                mrp_id=cr.fetchone()
                cr.execute(add_mrp_bom_sel, (slider_line.tz_name.id, slider_line.tz_name.name, product2, mrp_id))
                cr.execute(max_mrp_bom)
                max_mrp=cr.fetchone()
                cr.execute(query_mrp_bom_line, (mrp_id, 0))
                for bom_line in cr.fetchall():  # 遍历BOM明细中的数据
                    names = bom_line[16] + slider_line.slider_pian.name
                    pro_max_id=bom_line[2]
                    if bom_line[17]==True:
                        cr.execute("select count(1) from product_template where name=%s and id!=%s", (names, 0))
                        if [x[0] for x in cr.fetchall()] == [0L]:
                            ####产品名称,链牙,色带,长度,虚拟物品，ID获取后，添加一个新的产品
                            cr.execute(add_product_template, (
                                names, None, None, 0,
                                bom_line[15], bom_line[15]))
                            cr.execute(max_pro_temp_id)
                            # 获取最近插入的哪一行记录的ID,并向product_product表中添加一行数据
                            liners2 = cr.fetchone()
                            cr.execute(add_product_product_sel, (names, liners2, bom_line[22]))
                            cr.execute("select id from product_product where product_tmpl_id=%s and id!=%s", (liners2[0], 0))
                            product_ids = cr.fetchone()
                            cr.execute(luxian, (liners2[0], bom_line[15]))  #####Coyp路线
                            cr.execute(rules, ('OP/000' + str(liners2[0]), product_ids, bom_line[22]))  ##生成在补货规则
                            cr.execute(caigou, (liners2[0], bom_line[22]))  ###添加采购商
                            cr.execute(max_pro_product_id)
                            pro_max_id=cr.fetchone()
                    cr.execute(add_mrp_bom_line_sel,
                                                   (pro_max_id, bom_line[10], max_mrp, bom_line[18]))
                        # 以下代码生成BOM
                    cr.execute("select count(1) from mrp_bom where product_tmpl_id =(%s) and id!=%s",
                                       (bom_line[15], 0))
                    line_mrp2 = [x[0] for x in cr.fetchall()]
                                # 判断是否有这个虚拟产品的虚拟BOM
                    if line_mrp2 != [0L]:
                        cr.execute("select count(1) from mrp_bom where NAME =%s and id!=%s",
                                               (names, 0))
                        ...
                        if [x[0] for x in cr.fetchall()] == [0L]:
                            ...
                            cr.execute("select id from product_product where product_tmpl_id=%s and id!=%s",
                                                   (virtua_count[0], 0))
                            addr_obj = cr.fetchone()
                            cr.execute(query_mrp_bom, (bom_line[15], 0))
                            # 循环遍历该产品的虚拟物品BOM信息，并生成真实真实BOM
                            for bom in cr.fetchall():
                                # 向BOM行中添加数据
                                cr.execute(add_mrp_bom_sel, (virtua_count[0], names, addr_obj[0], bom[18]))
                                ...
                                bom_id_max_t = cr.fetchone()  # 获取到最近添加的BOM
                                cr.execute(query_mrp_bom_line, (bom[18], 0))
                                for bom_line2 in cr.fetchall():  # 遍历BOM明细中的数据
                                    indexs = bom_line2[2]
                                    if bom_line2[19] == u'BT(本头)':
                                        cr.execute(sql_str,
                                                               (slider_line.slider_bentou.id, 0))
                                        indexs = cr.fetchone()
                                    if bom_line2[19] == u'LP(拉片)':
                                        cr.execute(sql_str,
                                                               (slider_line.slider_pian.id, 0))
                                        indexs = cr.fetchone()
                                    cr.execute(add_mrp_bom_line_sel,
                                                           (indexs, bom_line2[10], bom_id_max_t, bom_line2[18]))
            cr.commit()
        except Exception, e:
    ...
    def onchange_slider_name(self, cr, uid, ids, slider_name, context=None):
        ...
        if slider_name:
            res["tz_state"] = 'B'
            slider_tz = self.pool.get('product.template').browse(cr, uid, slider_name)  # 订单行
            if slider_tz.pro_typeid== u'TZDT(涂装拉头)':
                tz_name = slider_name
                res["tz_state"] = 'A'
            else:
                res["tz_name"] = None
            ...
            if [x[0] for x in cr.fetchall()] != [0L]:
                ...
                for bom_line in cr.fetchall():
                    if bom_line[0] == u'BT(本头)':
                        bentou = bom_line[1]
                    if bom_line[0] == u'LP(拉片)':
                        pian = bom_line[1]
                        ###'slider_bentou':
                        ### [('virtual_item_key', '=', bentou)],
                       ## 'slider_pian':
                         ## [('virtual_item_key', '=', pian)],
            domain = {'tz_name':
                          [('pro_slider', '=', tz_name)]}
        return {'value': res, 'domain': domain}


    def onchange_pian_name(self, cr, uid, ids, slider_pian, context=None):
        res = {}
        domain = {}
        slider = []
        size3 = ''
        ...
        size30 = ''
        if slider_pian:
            cr.execute("select c.pro_material,b.name from product_line a inner join product_tem_line b on\
                    a.tem_type_id=b.id inner join product_template c on a.type_id=c.id  where a.type_id=%s and c.id!=%s",
                       (slider_pian, 0))
            for bom_line in cr.fetchall():
                slider = bom_line[0]
                if bom_line[1] == u'3号':
                ...
                if bom_line[1] == u'30号':
                    size30 = bom_line[1]
                print("UUUUUUUUUUUUUUUUUUU",slider_pian)
            domain = {'name':
                          [('pro_material', '=', slider), ('pro_slider_active', '=', True),
                           ('pro_size', 'in', (size3, size4, size5, size7, size8, size10, size30))]}
        return {'value': res, 'domain': domain}
